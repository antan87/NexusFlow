  ...
  try {
    ...
    if (!analysis || analysis.size === 0) {
      const lines = [
        ...
        '> **Scope:** This graph covers cross-repo package dependencies for merge order only. Runtime and intra-repo contracts are not inferred; see scoped knowledge entries.',
        '',
        '## Repos',
        '',
        ...repos
    ...
    md.push('> **Refresh:** Run `nexusflow refresh`.');
    md.push('> **Scope:** This graph covers cross-repo package dependencies for merge order only. Runtime and intra-repo contracts are not inferred; see scoped knowledge entries.');
    md.push('');
    ...
    if (!hasEdges && contracts.length === 0) {
      ...
      if (repos.length === 1) {
        ...
        md.push('- **Non-Linear Iteration**: If unexpected constraints or gotchas emerge, record them with `nexusflow knowledge add` or MCP `add_knowledge`.');
