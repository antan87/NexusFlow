...
async function buildDesiredFiles(
  ...
  if (assistants.includes('codex')) {
    for (const agent of agents) {
      ...
      const bytes = Buffer.from(`# AUTO-GENERATED by ${BRAND_NAME} from the agent catalog — do not edit\n\n${serializeCodexAgentToml(agent)}`, 'utf-8');
...
export async function reconcileWorkspaceResources(
  ...
  return runMaterialization(async () => {
    ...
    const workspaceConfigDir = configDirInfo.path;
    if (await fse.pathExists(workspaceConfigDir)) {
      await assertNoLinkedPathComponents(canonicalWorkspace, workspaceConfigDir);
    } else {
      await fse.ensureDir(workspaceConfigDir);
    }
    await assertPathIsNotLink(workspaceConfigDir);

    const release = await acquireLock(path.join(workspaceConfigDir, 'resource-materialization.lock'), {
      staleMs: 120_000,
      timeoutMs: 30_000,
