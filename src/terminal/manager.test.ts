...
describe('backend-owned terminals', () => {
  ...
  it('preserves process and output through detach/reconnect, with one writer', async () => {
    ...
    expect(() => f.manager.attach('alice', 'workspace', id, other)).toThrow('already attached');
    f.output('hello'); f.manager.detach('alice', 'workspace', id, f.client); f.output(' while hidden');
    expect(f.terminate).not.toHaveBeenCalled();
    f.manager.attach('alice', 'workspace', id, other);
    expect(other.send.mock.calls.flat().join('')).toContain(' while hidden');
    expect(() => f.manager.control('alice', 'workspace', id, f.client, { type: 'input', data: 'stale' })).toThrow('no longer owns');
    f.manager.control('alice', 'workspace', id, other, { type: 'input', data: 'pwd\r' });
