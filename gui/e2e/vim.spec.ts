...
test.describe('Vim Navigation Mode', () => {
  ...
  test('is disabled by default and can be toggled on/off via backslash and in Settings', async ({ page }) => {
    // 1. Fresh page with empty storage: Vim mode is OFF by default
    await page.goto('/');
    ...
    // 4. Navigate to settings and toggle via Switch component
  ...
  test.describe('Active Navigation Features', () => {
    test.beforeEach(async ({ page }) => {
      await page.addInitScript(() => {
        localStorage.setItem('nf:vim-enabled', 'on');
      });
    });
    ...
    test('opens and closes help cheatsheet with ? and Esc', async ({ page }) => {
      ...
      await expect(helpModal.getByText(/NexusFlow — Vim/)).toBeVisible();

      // Press Escape to close
      await page.keyboard.press('Escape');
      await expect(helpModal).not.toBeVisible();

      // Click ? Help button on statusline
      await statusline.getByRole('button', { name: '? Help' }).click();
      await expect(helpModal).toBeVisible();

      // Click ✕ to close
      await helpModal.getByRole('button', { name: 'Close help' }).click();
      await expect(helpModal).not.toBeVisible();
