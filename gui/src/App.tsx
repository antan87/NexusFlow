...
function AppInner() {
  ...
  const [subTab, setSubTab] = useState<'overview' | 'workroom' | 'services' | 'changes' | 'sessions' | 'knowledge' | 'plan' | 'skills'>('overview');
  ...
  useEffect(() => {
    ...
    if (p.startsWith('/workspaces')) {
      ...
      const tab = parts[2];
      const valid = ['overview', 'workroom', 'sessions', 'services', 'changes', 'knowledge', 'plan', 'skills'];
