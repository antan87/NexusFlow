...
import {
  ...
  AlertTriangle, AlertCircle, Terminal, Cpu, FolderOpen, Sparkles, PlusCircle
} from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import type { NexusFlowConfig, RepoInfo, DetectedAI, DetectedEditor } from '../types.js';
import { API_BASE } from '../lib/apiBase.js';

type RepoSelection = RepoInfo & { existingBranch?: string };
type RepoBranches = { local: string[]; remote: string[] };
...
interface WizardPageProps {
  ...
  repos: RepoInfo[];
  setRepos: React.Dispatch<React.SetStateAction<RepoInfo[]>>;
  reposLoading: boolean;
  repoSearch: string;
  setRepoSearch: React.Dispatch<React.SetStateAction<string>>;
  selectedRepos: RepoSelection[];
  setSelectedRepos: React.Dispatch<React.SetStateAction<RepoSelection[]>>;
...
export function WizardPage(props: WizardPageProps) {
  ...
  const {
    ...
    repos, setRepos, reposLoading, repoSearch, setRepoSearch,
    selectedRepos, setSelectedRepos, showToast,
  ...
  const [repoBranches, setRepoBranches] = React.useState<Record<string, RepoBranches>>({});
  const [newProjectName, setNewProjectName] = React.useState('');
  const [creatingProject, setCreatingProject] = React.useState(false);

  const handleToggleRepo = (repo: RepoInfo) => {
    const already = selectedRepos.some((r) => r.path === repo.path);
    setSelectedRepos((prev) =>
      already ? prev.filter((r) => r.path !== repo.path) : [...prev, repo]
    );
    // Lazily load this repo's branches for the existing-branch dropdown.
    if (!already && !repoBranches[repo.path]) {
      fetch(`${API_BASE}/api/repos/branches?path=${encodeURIComponent(repo.path)}`)
        .then((res) => (res.ok ? res.json() : null))
        .then((branches) => {
          if (branches) setRepoBranches((prev) => ({ ...prev, [repo.path]: branches }));
        })
        .catch(console.error);
    }
  };
    ...
    <div className="max-w-4xl mx-auto">
      ...
      {activeStep === 1 && (
        <div className="bg-surface/40 border border-gray-800/80 rounded-xl p-8 shadow-xl backdrop-blur-sm">
          ...
          <div className="flex items-center gap-3 mb-6 p-4 border border-dashed border-gray-800 rounded-xl bg-gray-950/20">
                      ...
                      <div className="flex items-center gap-2 pl-7" onClick={(e) => e.stopPropagation()}>
                        ...
                        >
                          ...
                          {branches?.local.map((b) => (
                            <option key={`l-${b}`} value={b}>{b}</option>
                          ))}
                          {branches?.remote
                            .filter((b) => !branches.local.includes(b))
                            .map((b) => (
                              <option key={`r-${b}`} value={b}>{b} (remote)</option>
                            ))}
                        </select>
                      </div>
                    )}
