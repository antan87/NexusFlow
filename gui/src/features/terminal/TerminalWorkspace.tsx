...
export function TerminalWorkspace({ workspacePath, ...props }: ComponentProps<typeof TerminalPane> & { workspacePath: string }) {
  ...
  const handleSplitDrag = useCallback((event: React.PointerEvent) => {
    ...
    const onPointerMove = (move: PointerEvent) => {
      ...
      const leftPercent = ((move.clientX - bounds.left) / bounds.width) * 100;
      setSplitPercent(Math.round(Math.max(20, Math.min(85, 100 - leftPercent))));
      setIsExpanded(false);
    };
    ...
    <div ref={containerRef} className="relative flex min-h-0 flex-1 flex-row">
      ...
      {inspector && <>
        <div role="separator" tabIndex={0} aria-orientation="vertical" aria-valuenow={isExpanded ? 85 : splitPercent}
          ...
          className={`group z-20 -mx-1 flex w-2 cursor-col-resize select-none items-center justify-center bg-border/60 transition-colors hover:w-2.5 hover:bg-primary/50 focus-visible:bg-primary/70 focus-visible:outline-hidden ${isDragging ? 'w-2.5 bg-primary/60' : ''}`}
        ...
        <div style={{ width: isExpanded ? '85%' : `${splitPercent}%` }} className={`h-full min-h-0 min-w-0 border-l border-border ${isDragging ? '' : 'transition-[width] duration-150'}`}>
